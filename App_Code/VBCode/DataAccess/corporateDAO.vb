...
Namespace core
    ...
    Public Class corporate
        ...
        Public Function SetMFBPermissions(ByVal Username As String) As Integer

            Dim sqlstring As String
            Dim PermissionSet As DataSet = New DataSet
            Dim MFB_Id As Integer

            If Not Username Is Nothing Then

                sqlstring = "select s.UserID, f.MFB_Id" & vbCrLf &
                                "from dbo.Security s" & vbCrLf &
                                "Inner Join dbo.Maintenance_Flagboard f" & vbCrLf &
                                "ON s.UserID = f.UserID" & vbCrLf &
                                "where s.UserID = '" & Username & "';"

                If Not Util.FillDataSet(PermissionSet, "PermissionSet", sqlstring) Then
                    Return 0
                End If
                Dim userstring As String = PermissionSet.Tables(0).Rows(0)("UserID")
                If userstring.IndexOf("FB") >= 0 Or userstring.IndexOf("fb") >= 0 Then
                    MFB_Id = Convert.ToInt32(PermissionSet.Tables(0).Rows(0)("MFB_Id"))
